...
final class StrMacros implements RegistersMacros
{
    ...
    {
        Str::macro('firstAndLastWords', fn (string $string): string => Str::of($string)
            ->explode(' ')
            ->filter()
            ->values()
            ->whenNotEmpty(fn (Collection $words): string => trim($words->first().' '.$words->last()), fn (): string => $string));
    }
    ...
    public static function split(): void
    {
        Str::macro('split', function (array|string $value, string $delimiter = ','): Collection {
            ...
            return Collection::make($array)
                ...
                ->map(function ($item): float|int|string {
                    if (is_numeric($item)) {
    ...
    public static function extractors(): void
    {
        ...
        Str::macro('extract_floats', fn (string|array $value, string $delimiter = ','): Collection => Collection::wrap($value)
            ...
            ->flatMap(fn (string $v): array => explode($delimiter, $v))
            ->map(fn (string $v): string => trim($v))
            ->filter(fn (string $v): bool => is_numeric($v) && str_contains($v, '.'))
    ...
    {
        Str::macro('prefix', fn (array $items, ?string $prefix = null) => $prefix
            ? collect($items)->map(fn ($item): string => "$prefix.$item")->toArray()
            : $items);
    }
        ...
        Str::macro('linesToCollection', function (
            ...
            return collect($items)
                ...
                ->when($unique, function (Collection $items) {
                    ...
                    return $items->filter(function ($item) use (&$seen): bool {
                        $hash = md5($item);
                        if (isset($seen[$hash])) {
                            return false;
                        }
